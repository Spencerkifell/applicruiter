...
import { JobPosting, ResumeRanking } from '../../models';
...
export class RankingsComponent implements OnInit {
  ...
  constructor(
    ...
    this.valueChangesSubscription =
      this._dataService.sharedSelectedJobId.subscribe((value) => {
        if (!value) {
  ...
  setResumeRankings(): void {
    if (
      ...
      return;
    this.disabled = true;
    this.updatedRankedResumes = this._restService
      .rankResumes(Number(this.selectedJobId), this.selectedJob.description)
      .subscribe({
        next: (data: any) => {
          ...
          dataToUpdate.forEach((data) => {
            const foundResume = this.resumeRankingCollection.find(
              (resume) => resume.id == data[1]
            );
            if (foundResume)
              foundResume.similarity_score = parseFloat(
                parseFloat(data[0]).toFixed(6)
              );
          });
